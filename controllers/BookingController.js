...
//         return res.status(400).json({ status: 400, message: `Room ${roomDoc.roomNumber} is already booked` });
//       }
...
//     // ✅ Mark rooms as unavailable immediately
//     for (const r of rooms) {
//       await Room.findByIdAndUpdate(r.room, { isAvailable: false });
//     }
...
//     const bookingNotif = {
//       message: `New booking created for ${guestName} | Rooms: ${roomNumbers}`,
//       type: "room-booking",
//       user: {
//         id: req.adminId || null,
//         email: guestEmail || null,
//         phone: guestContact || null,
//       },
//     };
...
//     console.log("🔔 Notification emitted (booking):", bookingNotif);
...
const createBooking = async (req, res) => {
  try {
    const { guestName, guestContact, guestEmail, rooms, checkIn, checkOut, source, paymentStatus, totalAmount, depositAmount, notes } = req.body;
    ...
    for (const r of rooms) {
      const roomDoc = await Room.findById(r.room);
      if (!roomDoc) return res.status(404).json({ status: 404, message: `Room not found: ${r.room}` });
      ...
      if (overlapping) return res.status(400).json({ status: 400, message: `Room ${roomDoc.roomNumber} is already booked for selected dates` });
    ...
    const io = getIo();
    const roomNumbers = rooms.map(r => r.roomNumber).join(", ");
    const bookingNotif = { message: `New booking for ${guestName} | Rooms: ${roomNumbers}`, type: "room-booking", user: { id: req.adminId, email: guestEmail, phone: guestContact } };
    await Notification.create(bookingNotif);
    io.emit("new-booking", bookingNotif);
